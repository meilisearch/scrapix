...
export async function processBlockSplit($: CheerioAPI, documents: BlockDocument[], config: Config): Promise<BlockDocument[]> {
  if (!config.features || !config.features.block_split || !config.features.block_split.activated) {
    return documents;
  }
  ...
  let elems = $("main h1, main h2, main h3, main h4, main h5, main h6, main p, main td, main li, main span");
  if (elems.length === 0) {
  ...
  for (const elem of elems.toArray()) {
    ...
    if (tag.startsWith("H")) {
      ...
      if (text && Array.isArray(currentDocument.p) && !currentDocument.p.includes(text)) {
