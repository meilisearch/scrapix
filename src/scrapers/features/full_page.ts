...
export async function processFullPage($: CheerioAPI, url: string, config: Config): Promise<FullPageDocument> {
  ...
  for (const elem of elems.toArray()) {
    ...
    if (tag.startsWith("H")) {
      ...
      if (text && Array.isArray(currentBlock.p) && !currentBlock.p.includes(text)) {
