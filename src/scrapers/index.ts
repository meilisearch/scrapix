...
import { processPDF } from "./features/pdf";
...
export class Scraper {
  ...
  async get(url: string, $: CheerioAPI): Promise<void> {
    ...
    if (this.shouldProcessFeature(features.pdf, url)) {
      document = await processPDF($, document, this.config);
    }
  ...
  private shouldProcessFeature(feature: any, url: string): boolean {
    ...
    const isIncluded = includePages.some((pattern: string) => this.matchesPattern(url, pattern));
    if (!isIncluded) return false;

    // Check if URL matches any exclude pattern
    const isExcluded = excludePages.some((pattern: string) => this.matchesPattern(url, pattern));
    return !isExcluded;
  }
